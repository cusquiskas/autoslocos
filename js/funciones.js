...
function resuelveError(dat) {
  ...
  if (llamante === '') llamante = (resuelveError.caller.name === 'validaErroresCbk' ? ((typeof (resuelveError.caller.caller) === 'function') ? resuelveError.caller.caller.name + '<br>' : '') : resuelveError.caller.name + '<br>');
...
function validaErroresCbk(d, h) {
  ...
  if (d.length) {
    for (var i = 0; i < d.length; i++) {
      switch (d[i].type) {
        case 'required':
          toast({ tipo: 'error', msg: d[i].label + ': El valor no puede ser nulo', donde: ((h === true) ? '.modal-header' : '') });
          break;
        case 'invalid':
          toast({ tipo: 'error', msg: d[i].label + ': No es válido', donde: ((h === true) ? '.modal-header' : '') });
          break;
        case 'NaN':
          toast({ tipo: 'error', msg: d[i].label + ': No es un número', donde: ((h === true) ? '.modal-header' : '') });
          break;
        case 'NaD':
          toast({ tipo: 'error', msg: d[i].label + ': Formato fecha erróneo', donde: ((h === true) ? '.modal-header' : '') });
          break;
        default:
